...
class Client:
    ...
    def __init__(
        ...
        *,
        ...
        if not url:
            raise ValueError(
                "Client base_url is required (set REBUNO_URL or pass base_url=)"
            )
    ...
    async def get_step(self, execution_id: str, step_id: str) -> Step:
        resp = await self._request(
            "GET", f"/v0/executions/{execution_id}/steps/{step_id}"
        )
    ...
    async def list_steps(self, execution_id: str, *, status: str = "") -> list[Step]:
        ...
        resp = await self._request(
            "GET", f"/v0/executions/{execution_id}/steps", params=params
        )
        data = resp.json()
    ...
    async def grant_approval(
        self, approval_id: str, *, decided_by: str, rationale: str = ""
    ) -> None:
